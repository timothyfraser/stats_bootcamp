# Module D: Data Wrangling

Module D introduces data wrangling techniques for cleaning, transforming, filtering, and organizing data using dplyr in R and pandas in Python.

---

## Table of Contents

- 💻 [D1_code.R](D1_code.R)
- 💻 [D1_code.py](D1_code.py)
- 💻 [D2_code.R](D2_code.R)
- 💻 [D2_code.py](D2_code.py)
- 💾 [airlines.csv](airlines.csv)
- 💾 [airports.csv](airports.csv)
- 💾 [flights.csv](flights.csv)
- 💾 [flights_sample.csv](flights_sample.csv)
- 💾 [jp_emissions.csv](jp_emissions.csv)
- 💾 [planes.csv](planes.csv)
- 💾 [weather.csv](weather.csv)

---

## Links

- **Lessons**
    - 📊 [Slides: Lesson D1 - Data Wrangling at Scale](https://docs.google.com/presentation/d/17dqBd2j3mrVDJE6h8v2i83b0uxLBZCNG7ksYTGGIjvE/edit?usp=sharing)
    - 🎥 [Watch: Lesson D1.3 - Data Wrangling in Python with pandas](https://cornell.hosted.panopto.com/Panopto/Pages/Viewer.aspx?id=3a48c578-33a1-4a44-96ef-b25b00400a07)
- **Activities**
    - 🎯 [Activity: D1_code.R - Data Wrangling in R with dplyr](https://github.com/timothyfraser/stats_bootcamp/blob/main/D/D1_code.R)
    - 🎯 [Activity: D1_code.py - Data Wrangling in Python with pandas](https://github.com/timothyfraser/stats_bootcamp/blob/main/D/D1_code.py)
    - 🎯 [Activity: D2_code.R - Grouping & Reframing in R](https://github.com/timothyfraser/stats_bootcamp/blob/main/D/D2_code.R)
    - 🎯 [Activity: D2_code.py - Grouping & Reframing in Python](https://github.com/timothyfraser/stats_bootcamp/blob/main/D/D2_code.py)
- **Learning Checks**
    - ✅ [Quiz: Data Wrangling](https://canvas.cornell.edu/courses/75015/modules/items/2909843)
    - 🔍 [Review: Quiz Solutions - Data Wrangling](https://canvas.cornell.edu/courses/75015/modules/items/2937618)

---

![](../docs/image_icons.png)

---

← 🏠 [Back to Statistics Bootcamp Home](../README.md)
